...
function getVisitorStats($db) {
    ...
    // New visitors: First time ever seen (first_seen within timeframe)
    // Returning visitors: Seen before (first_seen before timeframe but has visits in timeframe)  
    // Total: All non-bot unique visitors in timeframe
    // Bots: All bot visitors in timeframe
    $stats_query = "
        ...
        SELECT 
            COUNT(DISTINCT CASE WHEN NOT is_bot THEN visitor_id END) as total_visitors,
            COUNT(DISTINCT CASE WHEN is_bot THEN visitor_id END) as bots,
            COUNT(DISTINCT CASE 
                WHEN NOT is_bot AND first_seen >= datetime('now', '$interval') 
                THEN visitor_id 
            END) as new_visitors,
            COUNT(DISTINCT CASE 
                WHEN NOT is_bot AND first_seen < datetime('now', '$interval') 
            ...
            (SELECT COUNT(*) FROM events WHERE created_at >= datetime('now', '$interval')) as total_events,
            (SELECT COUNT(*) FROM events WHERE created_at >= datetime('now', '$interval') AND event_type = 'pageview') as pageviews
        FROM visitor_stats
    ";
        ...
        SELECT 
            ...
            CASE 
                ...
                WHEN EXISTS (
                    ...
                    WHERE e2.visitor_id = e.visitor_id 
...
function getChartData($db) {
    ...
    $chart_query = "
        WITH visitor_period_stats AS (
            SELECT DISTINCT
                e.visitor_id,
                strftime('$groupBy', e.created_at) as time_period,
                v.is_bot,
                v.first_seen
            FROM events e
            JOIN visitors v ON e.visitor_id = v.visitor_id
